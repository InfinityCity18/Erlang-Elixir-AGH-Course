...
add_value_fail_test() ->
  ...
  M3 = pollution:add_value("Stacja 1", Time, "PM10", 36.3, M2),
  ?assertMatch({error, _}, pollution:add_value("Stacja 1", Time, "PM10", 46.3, M3)), % tutaj błedy
  ?assertMatch({error, _}, pollution:add_value("Stacja 1", Time, "PM10", 36.3, M3)),
  ?assertMatch({error, _}, pollution:add_value({1,1}, Time, "PM10", 46.3, M3)),
  ?assertMatch({error, _}, pollution:add_value({1,1}, Time, "PM10", 36.3, M3)).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_moving_mean_exact_test() ->
  M = pollution:add_station("Stacja 1", {1,1}, pollution:create_monitor()),

  BaseTime = {{2026,3,26},{12,0,0}},

  OneHourAgo = {{2026,3,26},{11,0,0}}, % weight = 23
  TwoHoursAgo = {{2026,3,26},{10,0,0}}, % weight = 22

  M1 = pollution:add_value("Stacja 1", OneHourAgo, "PM10", 10, M),
  M2 = pollution:add_value("Stacja 1", TwoHoursAgo, "PM10", 20, M1),

  % Expected:
  % (23*10 + 22*20) / (23 + 22)
  Expected = (23*10 + 22*20) / (23 + 22),

  Result = pollution:get_moving_mean({1,1}, "PM10", BaseTime, M2),

  ?assertEqual(Expected, Result).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
get_moving_mean_24h_filter_test() ->
  M = pollution:add_station("Stacja 1", {1,1}, pollution:create_monitor()),

  BaseTime = {{2026,3,26},{12,0,0}},

  Old = {{2026,3,25},{10,0,0}}, % > 24h ago
  Recent = {{2026,3,26},{11,0,0}}, % 1h ago

  M1 = pollution:add_value("Stacja 1", Old, "PM10", 100, M),
  M2 = pollution:add_value("Stacja 1", Recent, "PM10", 10, M1),

  % Only Recent counts → weight = 23 → mean = 10
  Result = pollution:get_moving_mean({1,1}, "PM10", BaseTime, M2),
