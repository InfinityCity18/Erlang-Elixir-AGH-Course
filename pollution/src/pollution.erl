...
% Funkcje pomocnicze
station_exists(StationName, Coords, Monitor) ->
    length(
        maps:keys(
            maps:filter(
                fun({InMapStationName, InMapCoords}, _) ->
                    (InMapStationName == StationName) or (InMapCoords == Coords)
                end,
                Monitor
            )
        )
...
get_stationname_from_coords(Coords, Monitor) ->
    get_stationname_from_coords_internal(Coords, maps:keys(Monitor)).
...
get_stationname_from_coords_internal(_, []) ->
    {error, "No station with such coordinates found"}.

get_coords_from_stationname(StationName, Monitor) ->
    get_coords_from_stationname_internal(StationName, maps:keys(Monitor)).
...
get_coords_from_stationname_internal(_, []) ->
    {error, "No station with such name found"}.
...
add_station(StationName, Coords, Monitor) ->
    case station_exists(StationName, Coords, Monitor) of
        false -> {monitor, Monitor#{{StationName, Coords} => []}};
    ...
    case station_exists(null, {C1, C2}, Monitor) of
        true ->
            ...
            case lists:member(Measurement, Data) of
                ...
                false -> {monitor, Monitor#{{StationName, {C1, C2}} => Data ++ [Measurement]}}
            end;
        false ->
...
add_value(StationName, Time, Type, Value, Monitor) ->
    case station_exists(StationName, null, Monitor) of
        true ->
            ...
            case lists:member(Measurement, Data) of
                true -> {error, "Measurement exists"};
                false -> {monitor, Monitor#{{StationName, Coords} => Data ++ [Measurement]}}
            end;
        false ->
            {error, "No such station"}
    end.
...
remove_value({C1, C2}, Time, Type, Monitor) ->
    case station_exists(null, {C1, C2}, Monitor) of
        true ->
            ...
            Data = maps:get({StationName, Coords}, Monitor),
            NewData = lists:filter(
                fun(#{time := InTime, type := InType}) ->
                    not ((InTime == Time) and (InType == Type))
                end,
                Data
            ),
            if
                length(Data) == length(NewData) -> {error, "Not remove because not found"};
                true -> {monitor, Monitor#{{StationName, Coords} => NewData}}
            end;
        false ->
            {error, "No such station"}
    end;
    ...
    case station_exists(null, Coords, Monitor) of
        true ->
            ...
            Filtered = lists:filter(
                fun(#{time := InTime, type := InType}) ->
                    ((InTime == Time) and (InType == Type))
                end,
                Data
            ),
            Mapped = lists:map(fun(#{value := Value}) -> Value end, Filtered),
            if
                length(Mapped) == 1 -> {value, lists:nth(1, Mapped)};
...
get_station_min({C1, C2}, Type, Monitor) ->
    ...
    case station_exists(null, Coords, Monitor) of
        true ->
            ...
            Mapped = lists:map(fun(#{value := Value}) -> Value end, Filtered),
            if
                length(Mapped) > 0 -> {value, lists:min(Mapped)};
...
get_daily_mean(Type, Day, Monitor) ->
    ...
    Filtered = lists:filter(
        fun(#{type := InType, time := {InDay, _}}) -> ((InType == Type) and (InDay == Day)) end,
        Flattened
    ),
    if
        length(Filtered) == 0 ->
            {error, "No measurements to take mean of"};
        true ->
            Mapped = [Value || #{value := Value} <- Filtered],
            {value, lists:sum(Mapped) / length(Mapped)}
    end.
